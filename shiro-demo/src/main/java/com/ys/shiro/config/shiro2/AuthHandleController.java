...
public class AuthHandleController {
    ...
    @PostMapping("/login")
    public String loginFault(@RequestParam("username") String loginAccount, String password, HttpServletRequest request, HttpServletResponse response) {
        response.setHeader("Access-Control-Allow-Origin", "*");
        ...
        if (account != null && account.equals(loginAccount)) {
            ...
            try {
                ...
                response.setStatus(200);
                subject.login(new MyJwtToken(loginAccount, password, null));
                // 登陆成功后，生成token
